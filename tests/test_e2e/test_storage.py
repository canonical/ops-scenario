...
def test_storage_get_null(no_storage_ctx):
    with no_storage_ctx.event("update-status", State()) as mgr:
        storages = mgr.charm.model.storages
        assert not len(storages)


def test_storage_get_unknown_name(storage_ctx):
    with storage_ctx.event("update-status", State()) as mgr:
        storages = mgr.charm.model.storages
        # not in metadata
...
def test_storage_usage(storage_ctx):
    ...
    with storage_ctx.event("update-status", State(storage=[storage])) as mgr:
