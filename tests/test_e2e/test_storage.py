...
def test_storage_get_null(no_storage_ctx):
    with no_storage_ctx(no_storage_ctx.on.update_status(), State()) as event:
        storages = event.charm.model.storages
        assert not len(storages)


def test_storage_get_unknown_name(storage_ctx):
    with storage_ctx(storage_ctx.on.update_status(), State()) as event:
        storages = event.charm.model.storages
        # not in metadata
